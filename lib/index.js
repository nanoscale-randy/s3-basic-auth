...
function sendResponse(_req, res, next, method='proxy', url) {
  const req = _req;
  if (method === 'redirect') {
...
function proto(config) {
  return function _s3BasicAuth(req, res, next) {
    ...
    if (req.get('Authorization') === auth) {
      const oldUrl = req.originalUrl.replace(/^\/+/g, '').replace(/(\?.*)$/, '').replace('@', '%40');
      const url = generatePresignedUrl(oldUrl, config);
